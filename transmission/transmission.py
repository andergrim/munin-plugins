...
name = os.getenv('name', host)
plugin_name = os.getenv('name', list(os.path.split(sys.argv[0]))[1])

def config():
    conf = Template("""multigraph ${plugin_name}_throughput
graph_title Transmission throughput for ${name}
...
graph_title Transmission activity for ${name}
...
""")
    print conf.safe_substitute(plugin_name=plugin_name, host=host, name=name)
    sys.exit(0)
